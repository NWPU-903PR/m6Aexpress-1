...
.dist_stopcodon <- function(methy_level_infor, txdbinfor,species){
  ...
  if(species=="human"){
    org_db <- org.Hs.eg.db
    tans_name <- select(org_db, keys=cds_name , columns = c("SYMBOL"),keytype="UCSCKG")
  }
  if(species=="mouse"){
    org_db <- org.Mm.eg.db
    tans_name <- select(org_db, keys=cds_name, columns = c("SYMBOL"),keytype="UCSCKG")
  ...
  if(species=="yeast"){
    ...
    tans_name <- select(org_db, keys=cds_name, columns = "GENENAME",keytype="ORF")
  }
  ...
  ##stop codon region
  stopcodon_trans <- select(org.Hs.eg.db, keys=names(stop_codon), columns = c("SYMBOL"),keytype=("UCSCKG"))
  ...
  stopcodon_select <- select(org.Hs.eg.db, keys=names(select_stopcodon), columns = c("SYMBOL"),keytype=("UCSCKG"))
