Get_express_data <- function(INPUT_BAM, TREATED_INPUT_BAM=character(0),annot_type="hg19",
                             species="human",isPairedEnd=FALSE,GENE_ANNO_GTF = NULL){
  Input_data <- c(INPUT_BAM, TREATED_INPUT_BAM)
  gene_count <- featureCounts(Input_data,useMetaFeatures=TRUE, annot.inbuilt=annot_type, isPairedEnd=isPairedEnd, annot.ext = GENE_ANNO_GTF)
