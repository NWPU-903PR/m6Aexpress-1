...
DE_methy_detect <- function(peak_inform,condition1, condition2, out_dir){
  peak_infor <- peak_inform[[1]]
  libsizes <- peak_inform[[2]]
  methy_read <- peak_infor[,grep("IP", colnames(peak_infor))]
  ##update
  CTL_methy_read <- as.data.frame(methy_read[,grep(condition1,colnames(methy_read))])
  ...
  CTL_unmethy_read <- as.data.frame(unmethy_read[,grep(condition1,colnames(unmethy_read))])
  ...
  select_DMpeak_site <- QNB_test[which((QNB_test$pvalue<0.05)==TRUE),]
  ...
  write.table(DM_peak_infor,file=paste(out_dir,"DM_peak.xls",sep="/"), sep="\t",row.names =TRUE,quote = FALSE)
