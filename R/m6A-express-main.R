m6A_Express <- function(express_data, treated_express_data=character(0),
  ...
  if(model=="auto"){
    ...
    paired_expr_methy <- match_expr_methy(gene_count_infor=gene_express_data, decay_methy=gene_methyintensity)
  ...
  if(model=="DE-DM"){
    ...
    DE_gene <- Select_DEgene(gene_express_data,cond1, cond2,num_cond1, num_cond2,
                             DE_CUTOFF_TYPE=DE_CUTOFF_TYPE,diff_gene_fdr=diff_gene_fdr,diff_gene_pvalue=diff_gene_pvalue)
    ##get DM peak site
    ...
    gene_methyintensity <- peakmethyleveldecay(peak_inform=DM_methy,txdbinfor=TXDB, species=species)
    
    ##match expression and methylation intensity
    print("Identify differential expression gene are regulated by differential methylation peak.")
    paired_expr_methy <- match_expr_methy(gene_count_infor=DE_gene, decay_methy=gene_methyintensity)
    if (is.na(pvalue)) {CUTOFF_TYPE="padj"} else  {CUTOFF_TYPE="pvalue"} 
    m6Areg_expr_gene <- m6A_Express_model(Input_file=paired_expr_methy,CUTOFF_TYPE=CUTOFF_TYPE,pvalue=pvalue,
                                          FDR=FDR, out_dir=OUTPUT_DIR)
