m6A_Express_model <- function(Input_file, negative=TRUE, out_dir){
  ...
  if(negative==FALSE){
    select_adjbeta <- padj_beta[padj_beta$padj<0.05,]
    write.table(select_adjbeta,file=paste(out_dir,"m6A-express_result.xls",sep="/"), sep="\t",row.names =TRUE,quote = FALSE)

  }
  if(negative==TRUE){
    neg_select <- select_adjbeta[select_adjbeta$Beta1<0,]
    write.table(neg_select,file=paste(out_dir,"m6A-express_negative.xls",sep="/"), sep="\t",row.names =TRUE,quote = FALSE)
